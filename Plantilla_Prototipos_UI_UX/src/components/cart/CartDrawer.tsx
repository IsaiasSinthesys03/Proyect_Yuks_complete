        ...
        <>
            ...
            >
                    ...
                    {subtotal === 0 ? (
                        ...
                        <>
                            ...
                            <div className="bg-slate-800/50 p-4 rounded-xl border border-slate-700 shadow-inner">
                                {!hasFreeShipping ? (
                                    <>
                                        ...
                                        {tierMultiplier < 1 && (
                                            <p className="text-[10px] text-[#ffce07] mt-2 font-bold">Beneficio {tier}: tu umbral bajó a ${THRESHOLD_FREE_SHIPPING.toFixed(2)}</p>
                                        )}
                                    </>
                                ) : (
                            ...
                            <div className="space-y-4">
                                {items.map((item: any) => (
                                    <div key={item.variantId} className="flex gap-4 bg-slate-900/50 p-3 rounded-xl border border-slate-800 hover:border-slate-700 transition-colors group">
                                        ...
                                        <div className="flex-1">
                                            <h4 className="text-sm font-bold text-white group-hover:text-[#03bbd3] transition-colors">{item.name}</h4>
                                            <p className="text-xs text-slate-500">Talla {item.size ?? 'Única'} · x{item.quantity}</p>
                                        ...
                                        <button
                                            ...
                                            className="text-slate-600 hover:text-[#ec1676] transition-colors self-start"
                                        >
                ...
                <div className="p-6 border-t border-slate-800 bg-slate-900/80 backdrop-blur-md">
                    ...
                    {subtotal > 0 && (
                        <label className="flex items-start gap-3 mt-4 mb-6 cursor-pointer group">
                            <div className="relative flex items-center justify-center mt-0.5">
                                <input type="checkbox" checked={legalAccepted} onChange={(e) => setLegalAccepted(e.target.checked)} required className="peer appearance-none w-4 h-4 border-2 border-slate-600 rounded bg-slate-900 checked:bg-[#96c93e] checked:border-[#96c93e] transition-colors cursor-pointer" />
                    ...
                    <button
                        disabled={subtotal === 0 || !legalAccepted}
